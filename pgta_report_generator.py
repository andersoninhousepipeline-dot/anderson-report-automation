...
class PGTADocxGenerator:
    ...
    def _add_methodology_page(self, doc):
        """Methods, Limitations, and References with headings kept together"""
        # Content sections
        sections = [
        ...
        for head, body, bullets in sections:
            ...
            if body:
                ...
                # If there are bullets coming, keep this paragraph with them
                if bullets:
                    p.paragraph_format.keep_with_next = True
            ...
            if bullets:
                for i, b in enumerate(bullets):
                    ...
                    self._set_paragraph_font(p, font_size=9)
                    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
                    # Keep all bullets together except the last one
                    if i < len(bullets) - 1:
                        p.paragraph_format.keep_with_next = True
            doc.add_paragraph()
    ...
    def _add_embryo_page(self, doc, patient_data, embryo_data):
        ...
        # 1. Banner [Total: 490pt] - Adjusted widths for better patient name space
        banner = doc.add_table(rows=2, cols=6)
        self._set_table_fixed_layout(banner)
        self._set_column_widths(banner, [88, 6, 169, 68, 6, 153])
